test_that("1D American put >= European put", {
  ...
  eur <- european_option_lv(s_0, k, tau, r_d, q, SigmaMat, "put",
                             s_min, s_max, n_s, n_t)
  am  <- american_option_lv(s_0, k, tau, r_d, q, SigmaMat, "put",
                             s_min, s_max, n_s, n_t,
...
test_that("1D American call on non-dividend stock equals European", {
  ...
  eur <- european_option_lv(s_0, k, tau, r_d, q, SigmaMat, "call",
                             s_min, s_max, n_s, n_t)
  am  <- american_option_lv(s_0, k, tau, r_d, q, SigmaMat, "call",
                             s_min, s_max, n_s, n_t,
                             lambda = 1e4, tolerance = 1e-8)
...
test_that("1D American put price is positive and finite", {
  ...
  price <- american_option_lv(s_0, k, tau, r_d, q, SigmaMat, "put",
                               s_min, s_max, n_s, n_t,
                               lambda = 1e4, tolerance = 1e-8)
