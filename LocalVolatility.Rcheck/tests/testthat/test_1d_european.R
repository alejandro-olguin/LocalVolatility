test_that("1D European call vs Black-Scholes closed-form", {
  ...
  pde <- european_option_lv(s_0, k, tau, r_d, r_f, SigmaMat, "call",
                             s_min, s_max, n_s, n_t)

  # Black-Scholes analytical
  d1 <- (log(s_0 / k) + (r_d - r_f + 0.5 * sigma^2) * tau) / (sigma * sqrt(tau))
  d2 <- d1 - sigma * sqrt(tau)
  bs <- s_0 * exp(-r_f * tau) * pnorm(d1) - k * exp(-r_d * tau) * pnorm(d2)
...
test_that("1D European put vs Black-Scholes closed-form", {
  ...
  pde <- european_option_lv(s_0, k, tau, r_d, r_f, SigmaMat, "put",
                             s_min, s_max, n_s, n_t)

  d1 <- (log(s_0 / k) + (r_d - r_f + 0.5 * sigma^2) * tau) / (sigma * sqrt(tau))
  d2 <- d1 - sigma * sqrt(tau)
  bs <- k * exp(-r_d * tau) * pnorm(-d2) - s_0 * exp(-r_f * tau) * pnorm(-d1)
...
test_that("1D European put-call parity holds", {
  ...
  call_p <- european_option_lv(s_0, k, tau, r_d, r_f, SigmaMat, "call",
                                s_min, s_max, n_s, n_t)
  put_p  <- european_option_lv(s_0, k, tau, r_d, r_f, SigmaMat, "put",
                                s_min, s_max, n_s, n_t)

  parity <- call_p - put_p - (s_0 * exp(-r_f * tau) - k * exp(-r_d * tau))
...
test_that("1D European with non-zero s_min works correctly", {
  ...
  price <- european_option_lv(s_0, k, tau, r_d, r_f, SigmaMat, "call",
                               s_min, s_max, n_s, n_t)
...
test_that("1D European rejects invalid sigma dimensions", {
  ...
  expect_error(
    european_option_lv(s_0, k, tau, r_d, r_f, bad_sigma, "call", 0, 200, n_s, n_t),
    "sigma"
  )
})
